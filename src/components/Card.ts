...
export class CatalogItem extends Card<CatalogItemStatus> {
    ...
    set status({ status, label }: CatalogItemStatus) {
        ...
        let statusClass = '';
        if (status === 'active') {
            statusClass = bem(this.blockName, 'status', 'active').name;
        } else if (status === 'closed') {
            statusClass = bem(this.blockName, 'status', 'closed').name;
        }
    
        this._status.className = `card__status ${statusClass}`.trim();
    }    
}
